 ...
 function multi(count,page_num,page,showCount,pageName){
    if($("#pagination")){  
        ...
        if(pagecount>pagesize){
            ...
            if(currentpage>1){  
                pagehtml+= '<li><a href="'+pageName+'.php?page='+(currentpage-1)+'&showCount='+count+'">   上一页</a></li>';
            }  
            if((currentpage - showCounts) > 0){
                pagehtml+= '<li><a href="'+pageName+'.php?page='+(1)+'&showCount='+count+'">...'+(1)+'</a></li>';
            }
            for(var i=0;i<counts;i++){  
                if(i>=(currentpage - showCounts) && i<(currentpage + showCounts)){  
                    if(i==currentpage-1){  
                        pagehtml+= '<li class="active"><a href="'+pageName+'.php?page='+(i+1)+'&showCount='+count+'">'+(i+1)+'</a></li>';
                    }else{
                        pagehtml+= '<li><a href="'+pageName+'.php?page='+(i+1)+'&showCount='+count+'">'+(i+1)+'</a></li>';
                    } 
                }  
            }  
            if((currentpage + showCounts)<counts){
                pagehtml+= '<li><a href="'+pageName+'.php?page='+(counts)+'&showCount='+count+'">...'+(counts)+'</a></li>';
                //pagehtml+= '<li><a>...</a></li>';  
            }
            if(currentpage<counts){  
                pagehtml+= '<li><a href="'+pageName+'.php?page='+(currentpage+1)+'&showCount='+count+'">下一页</a></li>';
            }
    ...
    $('#turnToPageBtn').click(function(){
		...
		window.location.href=pageName+'.php?page='+thisPage+'&showCount='+thisCount;
	}) 
